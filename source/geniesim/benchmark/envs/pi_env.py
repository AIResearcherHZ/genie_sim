...
class PiEnv(DummyEnv):
    ...
    def get_observation(self):
        ...
        if "Taks_T1" in self.robot_cfg:
            for name in TAKS_T1_DUAL_ARM_JOINT_NAMES:
                states.append(full_joint_states[name])
        elif "G1" in self.robot_cfg:
    ...
    def reset(self):
        ...
        while True:
            ...
            if self.robot_cfg == "Taks_T1_omnipicker":
                self.api_core.set_joint_positions(self.init_arm,joint_indices=[self.robot_joint_indices[v] for v in TAKS_T1_DUAL_ARM_JOINT_NAMES],is_trajectory=False)
                self.api_core.set_joint_positions(self.init_waist,joint_indices=[self.robot_joint_indices[v] for v in TAKS_T1_WAIST_JOINT_NAMES],is_trajectory=False)
                self.api_core.set_joint_positions(self.init_head,joint_indices=[self.robot_joint_indices[v] for v in TAKS_T1_HEAD_JOINT_NAMES],is_trajectory=False)
            ...
            if self.robot_cfg == "Taks_T1_omnipicker":
                ...
                for name in TAKS_T1_WAIST_JOINT_NAMES:
    ...
    def step(self, action):
        ...
        if self.robot_cfg == "Taks_T1_omnipicker":
            self.api_core.set_joint_positions([float(v) for v in action[:14]],joint_indices=[self.robot_joint_indices[v] for v in TAKS_T1_DUAL_ARM_JOINT_NAMES],is_trajectory=True)
